"""
SM4 (国密SM4) block cipher implementation.
Block size: 128 bits, Key size: 128 bits.
"""

# SM4 S-Box
SM4_SBOX = [
    ,
]

# System parameter FK
SM4_FK = [0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC]

# Constant key CK
SM4_CK = [
    0x00070E15, 0x1C232A31, 0x383F464D, 0x545B6269,
    0x70777E85, 0x8C939AA1, 0xA8AFB6BD, 0xC4CBD2D9,
    0xE0E7EEF5, 0xFC030A11, 0x181F262D, 0x343B4249,
    0x50575E65, 0x6C737A81, 0x888F969D, 0xA4ABB2B9,
    0xC0C7CED5, 0xDCE3EAF1, 0xF8FF060D, 0x141B2229,
    0x30373E45, 0x4C535A61, 0x686F767D, 0x848B9299,
    0xA0A7AEB5, 0xBCC3CAD1, 0xD8DFE6ED, 0xF4FB0209,
    0x10171E25, 0x2C333A41, 0x484F565D, 0x646B7279,
]

BLOCK_SIZE = 16


def _rotl(x, n):
    return ((x << n) & 0xFFFFFFFF) | (x >> (32 - n))


def _bytes_to_words(b):
    return [
        (b[i] << 24) | (b[i + 1] << 16) | (b[i + 2] << 8) | b[i + 3]
        for i in range(0, len(b), 4)
    ]


def _words_to_bytes(w):
    result = bytearray()
    for word in w:
        result.extend([
            (word >> 24) & 0xFF,
            (word >> 16) & 0xFF,
            (word >> 8) & 0xFF,
            word & 0xFF,
        ])
    return bytes(result)


def _tau(A):
    """Non-linear transformation τ: substitute each byte through S-box."""
    return (
        (SM4_SBOX[(A >> 24) & 0xFF] << 24)
        | (SM4_SBOX[(A >> 16) & 0xFF] << 16)
        | (SM4_SBOX[(A >> 8) & 0xFF] << 8)
        | SM4_SBOX[A & 0xFF]
    )


def _L(B):
    """Linear transformation L."""
    return B ^ _rotl(B, 2) ^ _rotl(B, 10) ^ _rotl(B, 18) ^ _rotl(B, 24)


def _L_prime(B):
    """Linear transformation L' for key expansion."""
    return B ^ _rotl(B, 13) ^ _rotl(B, 23)


def _T(A):
    """Round function transformation T = L ∘ τ."""
    return _L(_tau(A))


def _T_prime(A):
    """Key expansion transformation T' = L' ∘ τ."""
    return _L_prime(_tau(A))


def _key_expansion(key_bytes):
    """Expand 128-bit key into 32 round keys."""
    MK = _bytes_to_words(key_bytes)
    K = [MK[i] ^ SM4_FK[i] for i in range(4)]
    rk = []
    for i in range(32):
        val = K[i] ^ _T_prime(K[i + 1] ^ K[i + 2] ^ K[i + 3] ^ SM4_CK[i])
        rk.append(val)
        K.append(val)
    return rk


def _sm4_encrypt_block(block, rk):
    """Encrypt a single 16-byte block."""
    X = _bytes_to_words(block)
    for i in range(32):
        X.append(X[i] ^ _T(X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[i]))
    return _words_to_bytes([X[35], X[34], X[33], X[32]])


def _sm4_decrypt_block(block, rk):
    """Decrypt a single 16-byte block (reverse round keys)."""
    return _sm4_encrypt_block(block, rk[::-1])


def _pkcs7_pad(data, block_size=16):
    pad_len = block_size - (len(data) % block_size)
    return data + bytes([pad_len] * pad_len)


def _pkcs7_unpad(data):
    pad_len = data[-1]
    if pad_len < 1 or pad_len > 16:
        raise ValueError("Invalid PKCS7 padding")
    if data[-pad_len:] != bytes([pad_len] * pad_len):
        raise ValueError("Invalid PKCS7 padding")
    return data[:-pad_len]


def sm4_encrypt_cbc(plaintext, key, iv):
    """SM4-CBC encrypt. plaintext, key, iv are all bytes."""
    rk = _key_expansion(key)
    padded = _pkcs7_pad(plaintext)
    prev = iv
    ct = bytearray()
    for i in range(0, len(padded), BLOCK_SIZE):
        block = bytes(a ^ b for a, b in zip(padded[i:i + BLOCK_SIZE], prev))
        enc = _sm4_encrypt_block(block, rk)
        ct.extend(enc)
        prev = enc
    return bytes(ct)


def sm4_decrypt_cbc(ciphertext, key, iv):
    """SM4-CBC decrypt. Returns unpadded plaintext bytes."""
    rk = _key_expansion(key)
    prev = iv
    pt = bytearray()
    for i in range(0, len(ciphertext), BLOCK_SIZE):
        block = ciphertext[i:i + BLOCK_SIZE]
        dec = _sm4_decrypt_block(block, rk)
        pt.extend(bytes(a ^ b for a, b in zip(dec, prev)))
        prev = block
    return _pkcs7_unpad(bytes(pt))
